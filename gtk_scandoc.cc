...
std::vector<Gtk::Image *> imageWidgets;
extern std::vector<std::string> imagePaths;

MainWindow::MainWindow()
    : fileButton("Choose An Image"), fileButton1("Open Webcam"),
      mainBox(Gtk::Orientation::VERTICAL),
      buttonBox(Gtk::Orientation::HORIZONTAL),
      entryBox(Gtk::Orientation::VERTICAL),
      imageBox(Gtk::Orientation::HORIZONTAL),
      utilityBox(Gtk::Orientation::HORIZONTAL), clearButton("Clear"),
      saveButton("Save") {
  ...
  mainBox.append(s);
  mainBox.append(imageLabel);
  mainBox.append(imagePreview);
  mainBox.append(se);
  mainBox.append(utilityBox);
  entryBox.append(heightEntry);
  entryBox.append(widthEntry);
  buttonBox.append(fileButton);
  buttonBox.append(fileButton1);
  utilityBox.append(clearButton);
  utilityBox.append(saveButton);
  utilityBox.set_margin(10);
  clearButton.set_margin(10);
  saveButton.set_margin(10);
  clearButton.set_hexpand(10);
  saveButton.set_hexpand(10);
  imageBox.set_margin(10);
  imagePreview.set_child(imageBox);
  imagePreview.set_expand();
  fileButton.set_margin(10);
  fileButton.set_hexpand(10);
  fileButton1.set_margin(10);
  fileButton1.set_hexpand(10);
  fileButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onFileSelected));
  fileButton1.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::startWebcam));
  saveButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::savePDF));
  clearButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::clearData));
}
...
void MainWindow::startWebcam() {
  std::string path = scanImageToWebcam(convertToFloat(heightEntry.get_text()),
                                       convertToFloat(widthEntry.get_text()));
...
void MainWindow::savePDF() { convertToPDF(imagePaths); }
...
void MainWindow::onFileDialogResponse(int responseId,
                                      Gtk::FileChooserDialog *dialog) {
  switch (responseId) {
  case Gtk::ResponseType::OK: {
    auto fileName = dialog->get_file()->get_path();
    std::string path =
        scanImage(fileName, convertToFloat(heightEntry.get_text()),
                  convertToFloat(widthEntry.get_text()));
    auto aer = new Gtk::Image(path);
    aer->set_size_request(300, 300);
    aer->set_margin(10);
